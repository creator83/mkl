...
:Gpio(prt)
{
	...
	val.full = value;
	PORT_GPCLR_REG(PortBase[Gpio::prt]) = (val.half[0]<<16| Gpio::Output << PORT_PCR_MUX_SHIFT);
	PORT_GPCHR_REG(PortBase[Gpio::prt]) = (val.half[1]<<16| Gpio::Output << PORT_PCR_MUX_SHIFT);
...
:Gpio(prt)
{
	...
	val.full = value;
	PORT_GPCLR_REG(PortBase[Gpio::prt]) = (val.half[0]<<16| mx << PORT_PCR_MUX_SHIFT);
	PORT_GPCHR_REG(PortBase[Gpio::prt]) = (val.half[1]<<16| mx << PORT_PCR_MUX_SHIFT);
}
